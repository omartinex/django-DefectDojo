...
This is the recommended way to connect Microsoft Entra ID: point
`DD_SOCIAL_AUTH_OIDC_OIDC_ENDPOINT` at `https://login.microsoftonline.com/<tenant-id>/v2.0`
and the rest is discovered automatically. There is also a dedicated
[Azure Active Directory](/admin/sso/os__azure_ad/) backend, kept for parity with existing
2.x configurations.

{{% alert title="Login only" color="warning" %}}
Mapping identity-provider groups onto DefectDojo User Groups is **not** implemented; the
group-related settings documented for DefectDojo 2.x have no effect. Use
[Authorized Users](/admin/user_management/os__authorized_users/) for access control.
{{% /alert %}}
