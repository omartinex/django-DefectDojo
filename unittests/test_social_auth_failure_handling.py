...
class TestSocialAuthIntegrationFailures(DojoTestCase):
    ...
    Deliberately NOT using override_settings for AUTHENTICATION_BACKENDS: running against
    the real tuple that dojo.sso.settings.apply_sso_settings installs is what makes this
    a wiring test. GithubOAuth2 from 2.58.4 is gone — this port ships the eight OAuth2
    backends only, and GitHub is the Enterprise variant.
    """

    BACKEND_CLASS_PATHS = {
        "github-enterprise": "social_core.backends.github_enterprise.GithubEnterpriseOAuth2",
